      ...
      async run(ctx) {
        ...
        // 1.2. Проверка на то, что команда выполняется на нужном сервере.
         if (ConfigManager.data.chat_id !== ctx.message?.chat.id)
           return ctx.reply("В этой группе нельзя использовать команды модерации.");
        ...
        if (message.from.id === bot.botInfo?.id)
          return ctx.reply("Что? Меня за что?");

        if (message.from.id === ctx.message.from.id)
          return ctx.reply("Самокритично, однако. Но я не позволю.")

        // 6. Проверка на то, чтобы юзером не был Админ.
        let userAdmin = chatAdministrators.find(c => c.user.id === message.from.id);
        if (userAdmin && chatAdministrators.find((c) => c.status == 'creator')?.user.id !== ctx.message.from.id)
          return ctx.reply("Только глава может давать варны Администраторам!");
